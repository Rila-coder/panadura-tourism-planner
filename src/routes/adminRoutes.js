...
// Use Vercel's writable /tmp directory in production, or public/uploads in local development
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Missing required fields: title, description, category_id, latitude, longitude, address",
      });
    }
...
router.put("/places/:id", async (req, res) => {
  try {
    ...
    res.json({
      ...
      data: {
        old_image_deleted:
          oldImageUrl && image_url && oldImageUrl !== image_url ? true : false,
      },
    });
  } catch (error) {
...
router.delete("/places/:id", async (req, res) => {
  try {
    ...
    res.json({
      ...
      data: {
        image_deleted:
          imageUrl && imageUrl.startsWith("/uploads/") ? true : false,
      },
    });
  } catch (error) {
  ...
  try {
    ...
    if (check[0].count > 0) {
      return res.status(400).json({
        success: false,
        message:
          "Cannot delete category with existing places. Remove places first.",
      });
    }
